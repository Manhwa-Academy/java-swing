...
public class MenuTaskbar extends JPanel {
    ...
    public void refreshUserInfo() {
        infoPanel.removeAll();
        this.nhanVienDTO = new NhanVienDAO()
                .selectById(String.valueOf(nhanVienDTO.getManv()));
        in4(infoPanel);
    ...
    public BufferedImage makeRoundedImage(Image image, int diameter) {
    ...
    public void in4(JPanel info) {
        ...
        // ====== ƯU TIÊN AVATAR ======
        String avatar = nhanVienDTO.getAvatar();
        if (avatar != null && !avatar.trim().isEmpty()) {
            java.io.File imgFile = new java.io.File("img/avatar/" + avatar);
            if (imgFile.exists()) {
                Image img = new ImageIcon(imgFile.getAbsolutePath()).getImage();
                BufferedImage roundedImg = makeRoundedImage(img, 50);
                lblIcon.setIcon(new ImageIcon(roundedImg));
            }
        }

        // ====== FALLBACK: ICON GIỚI TÍNH ======
        ...
        // ====== CLICK AVATAR → MỞ MY ACCOUNT ======
        lblIcon.addMouseListener(new MouseAdapter() {
            @Override
            public void mousePressed(MouseEvent evt) {
                new MyAccount(owner, MenuTaskbar.this,
                        "Chỉnh sửa thông tin tài khoản", true);
            }
        });
    }
}